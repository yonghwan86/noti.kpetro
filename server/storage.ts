...
export interface IStorage {
  ...
  getPersonalTasksSharedWithUser(
    userId: string,
    teamId: string,
  ...
  autoCompletePastTasks(today: string): Promise<number>;
}
...
export class PostgresStorage implements IStorage {
  async getOrCreateTeam(department: string, name: string): Promise<Team> {
    const existing = await db
      .select()
      .from(teams)
      .where(or(eq(teams.department, department), eq(teams.department, null)));
    // filter manually because or logic above is a bit loose
    const exactMatch = existing.find(
      (t) => t.department === department && t.name === name,
    );
  ...
  async deleteTeam(id: string): Promise<void> {
    const referencedUsers = await db
      .select({ id: users.id })
      .from(users)
  ...
  async deleteUser(id: string): Promise<void> {
    ...
    const referencedLogs = await db
      .select({ id: inspectionLogs.id })
      .from(inspectionLogs)
    ...
    return rows.map((asset) => ({
      ...
      status:
        asset.status === "suspended"
          ? "suspended"
          : calculateStatus(asset.nextDueDate),
    }));
  }
  ...
  async updateAsset(
    ...
    if (updates.inspectionCycleDays || updates.lastInspectedDate) {
      const cycleDays =
        updates.inspectionCycleDays ?? asset.inspectionCycleDays;
  ...
  async getPersonalTasksSharedWithUser(
    userId: string,
    teamId: string,
    department: string | null,
  ): Promise<PersonalTask[]> {
  ...
  async autoCompletePastTasks(today: string): Promise<number> {
    const allTasks = await db
      .select()
      .from(personalTasks)
      .where(eq(personalTasks.completed, false));

    const pastTaskIds: string[] = [];
    for (const task of allTasks) {
      const startDate = task.scheduledAt.split("T")[0];
      const endDate = task.scheduledEndAt ?? startDate;
      if (endDate < today) {
        pastTaskIds.push(task.id);
      }
    }

    if (pastTaskIds.length === 0) return 0;

    await db
      .update(personalTasks)
      .set({ completed: true })
      .where(inArray(personalTasks.id, pastTaskIds));

    return pastTaskIds.length;
  }
}
