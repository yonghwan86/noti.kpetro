...
export const api = {
  ...
  assets: {
    ...
    suspend: async (id: string, reason: string): Promise<Asset> => {
      const res = await fetch(`${API_BASE}/assets/${id}/suspend`, {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify({ reason }),
        ...fetchOptions,
      });
      return handleResponse(res, "Failed to suspend asset");
    },
    resume: async (id: string): Promise<Asset> => {
      const res = await fetch(`${API_BASE}/assets/${id}/resume`, {
        method: "POST",
        headers: getAuthHeaders(),
        ...fetchOptions,
      });
      return handleResponse(res, "Failed to resume asset");
    },
    batchInspect: async (assetIds: string[], date: string): Promise<{ success: boolean; count: number; assets: Asset[] }> => {
      const res = await fetch(`${API_BASE}/assets/batch-inspect`, {
        method: "POST",
        headers: getAuthHeaders(),
        body: JSON.stringify({ assetIds, date }),
        ...fetchOptions,
      });
      return handleResponse(res, "Failed to batch inspect assets");
    },
    delete: async (id: string): Promise<void> => {
      const res = await fetch(`${API_BASE}/assets/${id}`, { 
        method: "DELETE",
  ...
  history: {
    getByAsset: async (assetId: string): Promise<AssetHistory[]> => {
      const res = await fetch(`${API_BASE}/assets/${assetId}/history`, fetchOptions);
      return handleResponse(res, "Failed to fetch history");
    },
    getAll: async (categoryId?: string): Promise<AssetHistory[]> => {
      const url = categoryId ? `${API_BASE}/history?categoryId=${categoryId}` : `${API_BASE}/history`;
